...
public class MovementBotUtil {
    ...
    /**
     * Метод, создающий список возмиожных игровых состояний, к которым могут привести комплексные ходы.
     * @param gameState начанльное игровое состояние
     * @param complexMoves список комплексных ходов
     * @return список возможных состояний
     */
    public static List<State> collectPossibleStatesFromComplexMoves(GameState gameState, List<List<MakeMoveEvent>> complexMoves) {
        List<State> possibleStates = new ArrayList<>();
        for (List<MakeMoveEvent> complexMove : complexMoves) {
            collectPossibleStatesRecursive(new State(gameState.getCopy(), 1, complexMove), new ArrayList<>(complexMove), possibleStates);
        }
        return possibleStates;
    }
     ...
     */
    private static void collectPossibleStatesRecursive(State state, List<MakeMoveEvent> complexMove, List<State> result) {
        ...
        collectPossibleStatesRecursive(new State(hitState, state.getProbability()*hitProbability), new ArrayList<>(complexMove), result);
        collectPossibleStatesRecursive(new State(hitState, state.getProbability()*mistakeProbability), new ArrayList<>(complexMove), result);
    }
