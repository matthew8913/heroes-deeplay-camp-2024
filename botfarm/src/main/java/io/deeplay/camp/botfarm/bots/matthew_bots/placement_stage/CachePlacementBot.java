...
public class CachePlacementBot extends PlacementBot {
    ...
    /**
     * Стэк с уже сгенерированной расстановкой. Необходим, так как взаимодействие с игрой происходит через ивенты.
     */
    ...
    private Map<Unit[][], Unit[][]> loadCounterPlacements() {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader("botfarm/src/main/resources/counter-placements.csv"))) {
            String line;
    ...
    private List<Unit[][]> loadBestFirstPlayerPlacements() {
        ...
        ObjectMapper objectMapper = new ObjectMapper();
        try (BufferedReader br = new BufferedReader(new FileReader("botfarm/src/main/resources/top-first-placements.json"))) {
            String line;
            while ((line = br.readLine()) != null) {
    ...
    public PlaceUnitEvent generatePlaceUnitEvent(GameState gameState) {
        ...
        if (placeUnitEvents.isEmpty()) {
            if (gameStateCopy.getCurrentPlayer() == PlayerType.FIRST_PLAYER) {
                Unit[][] placement = bestFirstPlayerPlacements.get(new Random().nextInt(bestFirstPlayerPlacements.size()));
                ...
                if (counterPlacements.containsKey(enemyPlacement)) {
                    placeUnitEvents = generateEventsFromPlacements(counterPlacements.get(enemyPlacement), PlayerType.FIRST_PLAYER);
                } else {
     ...
     * @return стэк ивентов.
     */
    private Stack<PlaceUnitEvent> generateEventsFromPlacements(Unit[][] placement, PlayerType playerType) {
        Stack<PlaceUnitEvent> events = new Stack<>();
        for (int col = 0; col < Board.COLUMNS; col++) {
            for (int row = 0; row < Board.ROWS / 2; row++) {
                int eventRow = PlayerType.FIRST_PLAYER == playerType ? row : row + Board.ROWS / 2;
                events.add(new PlaceUnitEvent(col, eventRow, placement[col][row].getCopy(), playerType, col != 0 || row != 0, placement[col][row].isGeneral()));
            }
        }
        return events;
    }
